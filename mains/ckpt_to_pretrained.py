...
def my_app(
    ...
    dataset_config_paths = [
        Path(dataset_directory).absolute() / "config.yml"
        for dataset_directory in copy(checkpoint["datamodule_hyper_parameters"]["dataset_directories"])
    ]

    # label_to_character_list を datamodule_hyper_parameters から hyper_parameters に移動
    checkpoint["hyper_parameters"]["label_to_character_list"] = copy(checkpoint["datamodule_hyper_parameters"]["label_to_character_list"])
    ...
    for key in copy(list(checkpoint)):
        if key == "state_dict":
            ...
            for val in list(checkpoint[key]):
                if val not in ["image_size", "in_channels", "num_blocks", "channels", "num_classes", "block_types", "label_to_character_list"]:
    ...
    with zipfile.ZipFile(str(output_config_path), "a") as zf:
        ...
        # 学習に使用したデータセットの生成設定と文字情報
        for dataset_config_path in dataset_config_paths:
            zf.write(str(dataset_config_path))
            zf.write(str(dataset_config_path.with_suffix(CharactersDatabase.suffix())))
